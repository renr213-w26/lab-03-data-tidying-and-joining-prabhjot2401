...
country_inflation has 44 rows and each row represents a country. It has 30 columns each representing inflation rate in the country in that year.

b.  Display a list of the countries included in the dataset.
```{r}
country_inflation|>pull(country)
```
...
country_inflation |>
  ...
  slice(1:3)
```
The top three countries with the highest inflation rates in 2021 were Argentina, Turkiye, and Brazil. Argentina had an inflation rate of approximately 48.4%, Turkiye around 19.6%, and Brazil around 8.3%. In comparison, the United States had a relatively moderate inflation rate of about 4.7% in 2021, significantly lower than these three countries, particularly Argentina and Turkey.
...
```
The country with the largest inflation change over this time period is New Zealand, with an inflation ratio of approximately 3. This indicates that inflation increased between 1993 and 2021, as the ratio is greater than 1.
...
```
The reshaped data frame `country_inflation_long` has 1,276 rows and 3 columns: `country`, `year`, and `annual_inflation`. 
...
```
The lowest inflation rate observed was in Ireland in 2009
...
```
The highest inflation rate was observed in Brazil in 1994 at approximately 2076% while the lowest inflation rate was observed in Ireland in 2009 at approximately -4.5%.
...
I chose these five countries as they represent some of the world's largest economies across continents.
...
```{r}
country_inflation_filtered <- country_inflation_long |>
  filter(country %in% countries_of_interest)
country_inflation_filtered
country_inflation_filtered |>
  distinct(country)
```
...
ggplot(
  ...
  labs(
    title = "Annual Inflation Rates Across Major World Economies",
    subtitle = "1993-2021",
    x = "Year",
...
The `us_inflation` dataset has 4 columns and 132 rows . The variables are: `country` (always "United States"), `cpi_division_id` (a numeric identifier from 1-12), `year` (the year of observation), and `annual_inflation` (the annual inflation rate for that division and year).
...
```
The `cpi_divisions` dataset has 2 columns and 12 rows. The variables are: `id` (a numeric identifier from 1-12 matching the CPI division codes) and `description` (a text description of what each division represents, such as "Food and non-Alcoholic beverages".
...
```
The joined dataset `us_inflation_joined` has 5 columns and 132 rows. The columns are: `country`, `cpi_division_id`, `year`, `annual_inflation`, and `description`. A left join was most appropriate because we want to keep all the inflation data and add the descriptive labels from `cpi_divisions`.
...
```
I chose these five divisions because they represent essential spending categories that most households face.
...
```{r}
us_inflation_filtered <- us_inflation_joined |>
  filter(description %in% divisions_of_interest)
us_inflation_filtered
...
ggplot(
  ...
  geom_point(size = 2.5) +
  labs(
    title = "Annual Inflation Rates by CPI Division in the United States",
    subtitle = "1993-2021",
    x = "Year",
    y = "Annual Inflation Rate (%)",
    color = "CPI Division",
